...
class MelodySelector:
    def __init__(self, window_size=16, batch_size=200):         # Adjusted window size and batch size depending on the memory
        # Define the GP kernel with specific length scale bounds, higher is more flexible
        kernel = RBF(length_scale=0.1, length_scale_bounds=(1e-6, 1e2))

        # Initialize the Gaussian Process Regressor
        self.gp = GaussianProcessRegressor(
            kernel=kernel,
            alpha=1e-3,                     # Small noise level to avoid numerical issues
            random_state=42,
            optimizer='fmin_l_bfgs_b',      # Optimizer to use, L-BFGS-B is efficient for GP
            n_restarts_optimizer=3,         # Number of restarts for optimizer
    ...
    def prepare_training_data(self, data_frame):
        ...
        data_frame['Data'] = data_frame['normalized_pitch_sequence'].apply(lambda x: np.array([round(float(i),4) for i in x.strip('[]').split(',')]))
